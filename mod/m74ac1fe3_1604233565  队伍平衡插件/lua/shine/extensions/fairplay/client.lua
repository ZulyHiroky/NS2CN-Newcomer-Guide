...
local screentext_resetgame
local screentext_gamestate
local screentext_current
local screentext_JoinM
local screentext_JoinA
Plugin.screentext_resetgame=screentext_resetgame
Plugin.screentext_gamestate=screentext_gamestate
...
Plugin.resetgame_color= { 0, 150, 255}
Plugin.gamestate_color= { 0, 150, 255}
...
function Plugin:Initialise()
	...
	function MainMenu_OnCloseMenu()
	  ...
	  if((teamnumber == 0 or teamnumber == -1) and self.screentext_gamestate ~= nil and self.screentext_current ~= nil and self.screentext_JoinM ~= nil and self.screentext_JoinA ~= nil) then --RR
	      self.screentext_resetgame.Obj:SetIsVisible(true)
          self.screentext_gamestate.Obj:SetIsVisible(true)
	...
	function MainMenu_OnOpenMenu()
		  ...
		  if(self.screentext_gamestate ~= nil and self.screentext_current ~= nil and self.screentext_JoinM ~= nil and self.screentext_JoinA ~= nil) then
		      self.screentext_resetgame.Obj:SetIsVisible(false)
              self.screentext_gamestate.Obj:SetIsVisible(false)
...
function Plugin:UpdateResetinfoStatus()
    if (self.dt.seedingphase == true) then
        Resetinfo = self:GetPhrase( "INFO_RESET" )
        self.resetgame_color=self.NotifyBad
        self.screentext_resetgame =Shine.ScreenText.Add( "screentext_resetgame", {
				    X =0.6,
					Y = 0.3,
					Text = string.format("%s", Resetinfo),
					R = self.resetgame_color[1],
					G = self.resetgame_color[2],
					B = self.resetgame_color[3],
					Alignment = 0
		})
		self.screentext_resetgame.Obj:SetIsVisible(true)
	else
...
function Plugin:UpdateScreenTextStatus()
	if(self.dt.inform == true) then
		...
		if(teamnumber == -1 or teamnumber == 0) then
			...
			if(playerskill ~= -1 and playerskill ~= nil) then
				...
				local sumPlayers = totPlayersMarines + totPlayersAliens
				local gamesinfo = ""
				local canjoin = 0
				--if ( sumPlayers < self.dt.MinPlayers ) then
				if ( self.dt.seedingphase == true ) then
				    canjoin=self:GetCanJoinTeam(avgt1, avgt2, totPlayersMarines, totPlayersAliens, playerskill)
				    gamesinfo = self:GetPhrase( "INFO_SEED" )
				    self.gamestate_color=self.NotifyGood
				else
				    canjoin=self:CheckJoin(avgt1, avgt2, totPlayersMarines, totPlayersAliens, playerskill)
				    gamesinfo = self:GetPhrase( "INFO_GAME" )
				    self.gamestate_color=self.NotifyBad
				end
                ...
                self.screentext_gamestate =Shine.ScreenText.Add( "screentext_gamestate", {
				    X =0.6,
					Y = 0.4,
					Text = string.format("%s %d", gamesinfo, self.dt.MinPlayers),
					R = self.gamestate_color[1],
					G = self.gamestate_color[2],
					B = self.gamestate_color[3],
					Alignment = 0
				})
				self.screentext_gamestate.Obj:SetIsVisible(true)
					...
					self.screentext_current =Shine.ScreenText.Add( "screentext_current", {
								X =0.6,
								Y = 0.5,
								Text = string.format("%s                             Marines: %d [Delta: %d] Aliens: %d", self:GetPhrase( "TEXT_CURRENT" ), avgt1, deltaCurrent, avgt2),
					...
					self.screentext_JoinM =Shine.ScreenText.Add( "screentext_JoinM", {
										X =0.6,
										Y = 0.55,
										Text = string.format("%s  Marines %d [Delta: %d]  Aliens: %d", self:GetPhrase( "TEXT_JOIN_M" ), newavgt1,deltaT1,avgt2),
					...
					self.screentext_JoinA =Shine.ScreenText.Add( "screentext_JoinA", {
										X =0.6,
										Y = 0.6,
										Text = string.format("%s     Marines %d [Delta: %d] Aliens: %d", self:GetPhrase( "TEXT_JOIN_A" ), avgt1, deltaT2,newavgt2),
...
function Plugin:ReceiveDisplayResetinfo( Data )
    Print("Received Resetinfo")
    if(Data.show == true) then
		self:UpdateResetinfoStatus()
		Print("Received Resetinfo: data: show")
	else
		self.screentext_resetgame.Obj:SetIsVisible(false)
		Print("Received Resetinfo: data: false")
	end
end
...
function Plugin:Cleanup()
    Shine.ScreenText.Remove("screentext_resetgame")
	self.screentext_resetgame = nil
	
	Shine.ScreenText.Remove("screentext_gamestate")
	self.screentext_gamestate = nil
