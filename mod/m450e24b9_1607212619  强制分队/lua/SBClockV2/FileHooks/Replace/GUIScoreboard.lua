...
GUIScoreboard.kClickForMouseBackgroundSize = Vector(GUIScale(250), GUIScale(32), 0)
GUIScoreboard.kClickForMouseText = Locale.ResolveString("SB_CLICK_FOR_MOUSE")

GUIScoreboard.kSBClockBackgroundSize = Vector(GUIScale(250), GUIScale(32), 0)
...
if Client.GetOptionString("locale", "enUS") == "zhCN" then
    GUIScoreboard.kSBClockPattern = "%04d年%02d月%02d日 %02d:%02d:%02d" -- tostring(os.date("%m/%d/%y %H:%M:%S"))
else
    GUIScoreboard.kSBClockPattern = "%04d/%02d/%02d %02d:%02d:%02d" -- tostring(os.date("%m/%d/%y %H:%M:%S"))
end

GUIScoreboard.kSlidebarSize = Vector(7.5, 25, 0)
GUIScoreboard.kBgColor = Color(0, 0, 0, 0.5)
GUIScoreboard.kBgMaxYSpace = Client.GetScreenHeight() - ((GUIScoreboard.kClickForMouseBackgroundSize.y + 5) +  (GUIScoreboard.kSBClockBackgroundSize.y + 5) + (GUIScoreboard.kGameTimeBackgroundSize.y + 6) + 20)
...
GUIScoreboard.kSBClockFontName       = Fonts.kArial_17
...
function GUIScoreboard:Initialize()
    ...
    self.clickForMouseBackground:SetPosition(Vector(-GUIScoreboard.kClickForMouseBackgroundSize.x / 2, -GUIScoreboard.kClickForMouseBackgroundSize.y - GUIScoreboard.kSBClockBackgroundSize.y - 5, 0))
    ...
    self.SBClockBackground = GUIManager:CreateGraphicItem()
    self.SBClockBackground:SetSize(GUIScoreboard.kSBClockBackgroundSize)
    self.SBClockBackground:SetPosition(Vector(-GUIScoreboard.kSBClockBackgroundSize.x / 2, -GUIScoreboard.kSBClockBackgroundSize.y - 5, 0))
    self.SBClockBackground:SetAnchor(GUIItem.Middle, GUIItem.Bottom)
    self.SBClockBackground:SetIsVisible(false)

    self.SBClockIndicator = GUIManager:CreateTextItem()
    self.SBClockIndicator:SetFontName(GUIScoreboard.kSBClockFontName)
    self.SBClockIndicator:SetScale(GetScaledVector())
    GUIMakeFontScale(self.SBClockIndicator)
    self.SBClockIndicator:SetAnchor(GUIItem.Middle, GUIItem.Center)
    self.SBClockIndicator:SetTextAlignmentX(GUIItem.Align_Center)
    self.SBClockIndicator:SetTextAlignmentY(GUIItem.Align_Center)
    self.SBClockIndicator:SetColor(Color(0, 0, 0, 1))
    self.SBClockIndicator:SetText("_SB_Clock_")
    self.SBClockBackground:AddChild(self.SBClockIndicator)
...
function GUIScoreboard:Uninitialize()
    ...
    GUI.DestroyItem(self.SBClockIndicator)
    self.SBClockIndicator = nil
    GUI.DestroyItem(self.SBClockBackground)
    self.SBClockBackground = nil
...
local function HandleSlidebarClicked(self, _, mouseY)
    ...
    if self.slidebarBg:GetIsVisible() and self.isDragging then
        ...
        local bottomPos = Client.GetScreenHeight() - ((GUIScoreboard.kClickForMouseBackgroundSize.y + 5) + (GUIScoreboard.kSBClockBackgroundSize.y + 5)) - 19
...
function GUIScoreboard:Update(deltaTime)
    ...
    local SBClockbackgroundColor = PlayerUI_GetTeamColor()
    SBClockbackgroundColor.a = 0.8
    self.SBClockBackground:SetColor(SBClockbackgroundColor)
    local SBC_tt = os.date("*t")
    local SBC_datestr = string.format(self.kSBClockPattern,
                                      SBC_tt.year, SBC_tt.month, SBC_tt.day, SBC_tt.hour, SBC_tt.min, SBC_tt.sec)
    self.SBClockIndicator:SetText(SBC_datestr)
    self.SBClockBackground:SetIsVisible(vis)
    --First, update teams.
    local teamGUISize = {}
    ...
    if vis then
        ...
        local displaySpace = Client.GetScreenHeight() - ((GUIScoreboard.kClickForMouseBackgroundSize.y + 5) + (GUIScoreboard.kSBClockBackgroundSize.y + 5) + (GUIScoreboard.kGameTimeBackgroundSize.y + 6) + 20)
